...
with open('train_data.csv','rb') as train_data_file:
    ...
    train_lines=csv.reader(train_data_file)
...
print "classifying test data using mle estimates"
...
with open('test_data.csv','rb') as test_data_file:
    ...
    test_lines=csv.reader(test_data_file)
...
for class_id,class_val in train.items():
    ...
    for val in class_val:
        ...
        train_nk[class_id][val[1]]=train_nk_val[val[1]]
...
for class_id,class_val in train.items():
    ...
    for val in class_val:
        ...
        if ((train_nk[class_id][val[1]])/(total_n))!=0:
            ml_est_val[val[1]]=ml_est_val[val[1]]+math.log((float)(train_nk[class_id][val[1]])/(float)(total_n))
            ml_est[class_id][val[1]]=ml_est_val[val[1]]
...
for class_id,class_val in train.items():
    ...
    for val in class_val:
        ...
        if val[1] in doc_id_word[val[0]]:
            doc_class_prob_val[val[0]]=doc_class_prob_val[val[0]]+ml_est[class_id][val[1]]
